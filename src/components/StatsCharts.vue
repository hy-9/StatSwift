...
function exportFile() {
  ...
  //设置列宽
  let tempMaxCol: number = 8;
  let nameAmount: number = 0;
  const chineseRegex = /[\u4e00-\u9fa5\u3400-\u4DBF\uF900-\uFAFF]/g;
  sheetData.map((item) => {
    let chineseAmount = item.名称.match(chineseRegex)?.length;
    console.log(item.名称.length, chineseAmount);
    if (chineseAmount) {
      nameAmount = item.名称.length - chineseAmount + chineseAmount * 2;
    }
    if (nameAmount > tempMaxCol) {
      tempMaxCol = nameAmount;
    }
  });
  ws["!cols"] = [
    { wch: tempMaxCol }, // 第一列宽度为10个字符
  ];
